...
class ShufflingCardGrid extends Component<any, any> {
  constructor(props: any, context: any) {
    ...
    this.state = {
      centered: false,
      cardIndex: _.fromPairs(this.props.cards.map((c: any, index: any) => [c.key, index])),
  ...
  shuffle = () => {
    ...
    this.setState({
      cardIndex: _.fromPairs(newCards.map((c: any, index: any) => [c.key, index])),
    });
  ...
  componentWillReceiveProps(nextProps: any) {
    ...
    if (nextProps.stage !== this.props.stage) {
      updater.cardIndex = _.fromPairs(activeCards.map((c: any, index: any) => [c.key, index]));
    }
  ...
  render() {
    ...
    return (
      <div style={{ width: actualWidth }} className="ShufflingCardGrid">
